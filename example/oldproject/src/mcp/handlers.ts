...
export function createToolHandler(toolName: string, client: SDPClient | SDPClientV2): ToolHandler {
  ...
  const handlers: Record<string, ToolHandler> = {
    ...
    // Auth status handler
    check_auth_status: async (_args) => {
      // This is a special handler that checks OAuth status
      // In the self-client mode, this information comes from the session context
      return {
        message: "OAuth status check is handled at the server level. If you're seeing this message, your authentication is working.",
        authenticated: true,
        mode: "self-client"
      };
    },
